...
export function useKeyNavigation({
  root,
  restoreFocusOnOverflowBottom,
  restoreFocusOnOverflowTop,
  ...
  useEffect(() => {
    elements.current = root.current
      ? Array.from(root.current.querySelectorAll("[tabindex]"))
      : [];
  ...
  function getCurrentIndex() {
    ...
    if (currentIndex === -1) {
      currentIndex = document.activeElement
        ? elements.current.indexOf(document.activeElement)
        : -1;
    }
    return currentIndex;
  }
